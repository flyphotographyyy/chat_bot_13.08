...
def portfolio_walkforward_backtest(tickers: List[str], risk_profile: str, train_m:int=18, test_m:int=6, top_k:int=8, rebalance:str='W-MON', cost_bps:int=10, slip_bps:int=10) -> Dict:
    # Прост портфейлен OOS: дневни данни, score_simple, седмично ребалансиране към top-K BUY сигнали
    if not tickers: return {"oos_trades":0}
    # fetch all data
    data: Dict[str,pd.DataFrame] = {}
    for t in tickers:
        df = fetch_price_history(t, (train_m+test_m)*30*3, '1d')
        if df is None or df.empty: continue
        df = compute_indicators(df)
        data[t]=df
    if not data: return {"oos_trades":0}
    ...
    base_sell= {"conservative":35,"balanced":40,"aggressive":45}[risk_profile]

    start_i = train_m*21
    step = test_m*21
    equity = 1.0; peak=1.0; maxdd=0.0; trades=0; turnover=0.0
    for i in range(start_i, len(all_idx)-step, step):
        test_idx = all_idx[i:i+step]
        if len(test_idx)<5: break
        # weekly rebalance dates within test segment
        rebal_dates = pd.date_range(test_idx[0], test_idx[-1], freq=rebalance)
        held: Dict[str,float] = {}  # weight per ticker
        last_prices: Dict[str,float] = {}
        for d in test_idx:
            # on rebalance day: pick top-K BUY by score
            if d in rebal_dates:
                # compute today scores and prices
                cands = []
                for t,df in data.items():
                    if d not in df.index: continue
                    row = df.loc[d]
                    sc = _score_simple(row)
                    # regime approx using SPY SMA
                    # Use fixed thresholds (no VIX intra changes here)
                    if sc >= base_buy+2:
                        cands.append((t, sc, float(row['Close'])))
                cands.sort(key=lambda x: x[1], reverse=True)
                selected = [x[0] for x in cands[:top_k]]
                # calculate trades/turnover
                desired = {t: 1.0/max(1,len(selected)) for t in selected}
                # compute cost on weight changes
                for t in set(list(held.keys())+selected):
                    w_old = held.get(t,0.0); w_new = desired.get(t,0.0)
                    if abs(w_new - w_old) > 1e-6:
                        trades += 1
                        turnover += abs(w_new - w_old)
                held = desired
            # daily equity evolution
            if not held: continue
            # compute portfolio return for the day using close-to-close
            day_ret = 0.0
            for t,w in held.items():
                df = data.get(t)
                if df is None or d not in df.index: continue
                if last_prices.get(t) is None:
                    last_prices[t] = float(df.loc[d,'Close'])
                    continue
                px_prev = last_prices[t]; px_now = float(df.loc[d,'Close'])
                r = (px_now - px_prev) / px_prev if px_prev>0 else 0.0
                day_ret += w * r
                last_prices[t] = px_now
            # apply costs/slippage only on rebalance days implicitly via turnover approximation at the end of segment
            equity *= (1 + day_ret)
            peak=max(peak,equity); maxdd=max(maxdd,1-equity/peak)
        # apply transaction cost for the segment based on average turnover
        equity *= max(0.0, 1 - turnover * ((cost_bps+slip_bps)/1e4))
    n_years = max(1e-9, len(all_idx)/252)
    cagr = equity**(1/n_years) - 1 if equity>0 else -1
    sharpe = (cagr) / max(1e-9, 0.15)  # груба нормализация, без дневна std тук
    return {"oos_equity":equity, "oos_CAGR":cagr, "oos_maxDD":maxdd, "oos_trades":trades, "oos_turnover":turnover, "oos_sharpe":sharpe}
...
def main():
    ...
    if st.button("🚀 Run Enhanced Analysis", type="primary"):
        ...
        try:
            res_pf = portfolio_walkforward_backtest([r['ticker'] for r in results], risk_profile, CFG['wf']['train_months'], CFG['wf']['test_months'], CFG['wf']['top_k'], CFG['wf']['rebalance'], CFG['wf']['cost_bps'], CFG['wf']['slip_bps'])
            if res_pf.get('oos_trades',0) >= 0:
                st.caption(f"📦 Portfolio OOS: CAGR={res_pf.get('oos_CAGR',0):.2%} · maxDD={res_pf.get('oos_maxDD',0):.2%} · Sharpe~{res_pf.get('oos_sharpe',0):.2f} · turnover={res_pf.get('oos_turnover',0):.2f}")
        except Exception:
            pass
